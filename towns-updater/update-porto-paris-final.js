...
// Data that matches existing schema exactly
const portoDataFinal = {
  // Basic info
  cost_index: 1700,
  ...
  lifestyle_description: "Life in Porto unfolds at a relaxed pace, with mornings often starting at local cafés over strong coffee and fresh pastatas. Retirees here enjoy a rich social calendar including wine tastings in historic cellars, concerts at Casa da Música, and leisurely strolls along the Douro riverfront.",
  ...
  // Visa info
  visa_on_arrival_countries: ["USA", "UK", "Canada", "Australia"],
...
const parisDataFinal = {
  ...
  lifestyle_description: "Parisian retirement life revolves around neighborhood quartiers, each with distinct character and local markets. Days might include morning croissants at the corner café, afternoons in Luxembourg Gardens, and evenings at intimate bistros.",
...
async function updatePortoAndParisFinal() {
  ...
  if (portoError) {
    ...
    console.log('   Added: Cost data, ratings, infrastructure, language, visa info')
  }
  ...
  if (porto) {
    ...
    console.log('New data: visa_on_arrival_countries includes', porto.visa_on_arrival_countries?.length, 'countries')
  }
  ...
  if (paris) {
    ...
    console.log('New data: income_tax_rate_pct =', paris.income_tax_rate_pct + '%')
  }
  
  console.log('\n✨ Cities now have enhanced data including:')
  console.log('   - Income tax rates for budget calculations')

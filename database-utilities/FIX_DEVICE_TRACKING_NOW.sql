-- ============================================================================
-- FIX DEVICE TRACKING - DROP ALL VERSIONS AND RESTORE CORRECT ONE
-- ============================================================================
-- This fixes the duplicate function error and restores proper tracking
-- ============================================================================
...
DECLARE
    func_record RECORD;
BEGIN
    -- Find all versions of update_user_device
    FOR func_record IN
        SELECT
            'DROP FUNCTION IF EXISTS public.' || proname || '(' ||
    ...
    RAISE NOTICE '✅ All versions of update_user_device have been dropped';
END $$;

-- Now create the CORRECT version with all 46 parameters
...
-- Verify the correct function now exists
DO $$
DECLARE
    param_count INTEGER;
    func_params TEXT;
BEGIN
    ...
    INTO param_count, func_params
    FROM pg_proc
    WHERE proname = 'update_user_device'
    AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LIMIT 1;

    RAISE NOTICE '✅ DEVICE TRACKING RESTORED!';
    RAISE NOTICE '   Function: update_user_device';
    RAISE NOTICE '   Parameters: % total', param_count;
    RAISE NOTICE '   Ready to track like we did for Shabnam in Nova Scotia!';
END $$;
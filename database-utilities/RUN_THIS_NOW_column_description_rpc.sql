...
BEGIN
  ...
  IF update_column_description.table_name != 'towns' THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE c.table_schema = 'public'
      AND c.table_name = 'towns'
      AND c.column_name = update_column_description.column_name
  ) THEN
    RAISE EXCEPTION 'Column % does not exist in towns table', update_column_description.column_name;
  END IF;

  -- Update the column comment
  EXECUTE format(
    'COMMENT ON COLUMN public.towns.%I IS %L',
    update_column_description.column_name,
    update_column_description.new_description
  );
END;
$$;

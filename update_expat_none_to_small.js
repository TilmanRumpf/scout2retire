...
async function updateExpatNoneToSmall() {
  console.log('Updating expat_community_size: "none" → "small"...\n');
  
  // Update all "none" values to "small"
  const { data, error } = await supabase
    .from('towns')
    .update({ expat_community_size: 'small' })
    .eq('expat_community_size', 'none');

-- Create hobbies table with proper normalization
-- Scout2Retire - Hobbies Database Schema
...
-- Create the main hobbies table
CREATE TABLE hobbies (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    name TEXT NOT NULL UNIQUE,
    category TEXT NOT NULL CHECK (category IN ('activity', 'interest', 'custom')),
    description TEXT,
    icon TEXT, -- For storing icon names (e.g., 'Snowflake', 'Heart', etc.)
...
-- Hobbies table: Everyone can read, authenticated users can insert
CREATE POLICY "Hobbies are viewable by everyone" ON hobbies
    FOR SELECT USING (true);

CREATE POLICY "Authenticated users can insert hobbies" ON hobbies
    FOR INSERT WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Authenticated users can update hobbies" ON hobbies
    FOR UPDATE USING (auth.role() = 'authenticated');

-- User hobbies: Users can only see and manage their own hobbies
...
CREATE POLICY "Users can update their own hobbies" ON user_hobbies
    FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can delete their own hobbies" ON user_hobbies
    FOR DELETE USING (auth.uid() = user_id);

-- Town hobbies: Everyone can read, authenticated users can manage
CREATE POLICY "Town hobbies are viewable by everyone" ON town_hobbies
    FOR SELECT USING (true);

CREATE POLICY "Authenticated users can manage town hobbies" ON town_hobbies
    FOR ALL USING (auth.role() = 'authenticated');

-- Insert all the hobbies data from OnboardingHobbies.jsx

-- Activity options (9 activities)
...
-- All hobbies from the allHobbies array (114 hobbies)
INSERT INTO hobbies (name, category, description, icon) VALUES
...
-- Create a view for easy hobby management
CREATE VIEW hobbies_by_category AS
SELECT 
    category,
    COUNT(*) as count,
    array_agg(name ORDER BY name) as hobby_names
FROM hobbies 
GROUP BY category 
ORDER BY category;
...
GRANT SELECT, INSERT, UPDATE, DELETE ON user_hobbies TO authenticated;
GRANT SELECT ON town_hobbies TO anon, authenticated;
GRANT INSERT, UPDATE, DELETE ON town_hobbies TO authenticated;
GRANT EXECUTE ON FUNCTION get_user_hobbies_detailed(UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION get_town_hobbies_detailed(UUID) TO anon, authenticated;